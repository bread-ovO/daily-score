...
- 每天 23:55 自动发送今日日报
- 每周日 23:58 自动发送本周周报
- 支持手动查看日报、周报、月榜、累计榜和积分明细
- 日报、周报、月榜和累计榜使用 AstrBot 自带 HTML t2i 渲染为智人排行图片
- 支持指定用户清空当前群积分记录
...
加减分成功后会回复目标成员今日分、本周分和本次操作理由。操作者给自己扣分会被拦截。日报统计当天流水，周报统计本周一到周日流水，月榜统计当月流水，累计榜统计全部历史流水。日报、周报、月榜和累计榜默认按分数从高到低排列并渲染为图片，渲染失败时回复文本排行。积分明细默认展示今日流水，可通过 `weekly` 查看本周明细，通过 `total` 查看累计明细，在命令中 `@成员` 可只查看该成员明细。`/score reset` 和 `/score 重置` 清空当前群全部积分流水，保留群配置和成员名缓存。
